...
Extract cost-effectiveness variables from main individual-level data set.
...
costeff_cohort <-
  ...
  mutate(E_cost_sec_inf = mean_num_sec_inf * mean_cost.aTB_TxDx * all_secondary_inf_discounts,
         E_cost_notif_statusquo = (all_notif_discounts * mean_cost.aTB_TxDx) + E_cost_sec_inf,
         E_QALY_notif_statusquo = (cfr * QALY_fatality) + ((1 - cfr) * QALY_cured)) %>%
  stats::na.omit()
```
...
for (s in seq_len(n.scenarios)) {
  ...
  for (i in seq_len(N.mc)) {
    ...
    interv_cost[[i]] <- scenario_cost(interv$ENDPOINT_cost,
                                      unit_cost$aTB_TxDx,
                                      NUM_SECONDARY_INF,
                                      costeff_cohort,
                                      total_tb = n_all_tb,
    ...
    interv_QALY[[i]] <- scenario_QALY(avoided = avoid_tb[[s]][i, ],
                                      total = n_all_tb,
                                      costeff_cohort)
  }
