...
p <-
  grid.arrange(inmb_levelplot_25 + labs(subtitle = "A") + font_size + margin + guides(fill = guide_legend("INMB (£)")),
               inmb_levelplot_50 + labs(subtitle = "B") + font_size + margin + guides(fill = guide_legend("INMB (£)")) + ylab(""),
               inmb_levelplot_100 + labs(subtitle = "C") + font_size + margin + guides(fill = guide_legend("INMB (£)")) + ylab(""),
               prob_ce_gridplot_25 + labs(subtitle = "D") + font_size + theme(plot.margin = unit(c(0,1,0,0), "cm")) + guides(fill = guide_legend("Probability\ncost-effective")) + guides(fill = FALSE),
               ...
               prob_ce_gridplot_100 + labs(subtitle = "F") + font_size + theme(plot.margin = unit(c(0,0,0,-2), "cm")) + guides(fill = guide_legend("Probability\ncost-effective")) + ylab(""),
               out_25 + theme(text = element_text(size = 10)) + labs(subtitle = "G") + font_size + margin_ceac,
               out_50 + theme(text = element_text(size = 10)) + labs(subtitle = "H") + font_size + margin_ceac + ylab(""),
               out_100 + theme(text = element_text(size = 10)) + labs(subtitle = "I") + font_size + margin_ceac + ylab(""),
               nrow = 3)
