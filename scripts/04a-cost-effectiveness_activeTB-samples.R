...
if (!exists("p.complete_treat_scenarios")) {
  ...
  p.complete_treat_scenarios <-
    read.csv(file = paste(diroutput, "prob_complete_Tx_given_LTBI_by_who.csv", sep = "/"),
             header = FALSE) %>%
...
for (scenario_i in seq_len(n.scenarios)) {
  ...
  for (tbsample in uk_tbX_names) {
    ...
    uk_tbX_after_screen <- sample_uk_tb(prob = p.complete_treat_sample,
                                        is.tb = uk_tb_TRUE_year)
  ...
  n.tb_screen[[scenario_i]] <-
    uk_tb_scenarios %>%
    group_by(sim, status) %>%
    tally()

  # rename tb status
  n.tb_screen[[scenario_i]]$status <-
    plyr::revalue(x = as.factor(n.tb_screen[[scenario_i]]$status),
