...
##TODO: where is times_years data?
...
dataset_tb_times_all <-
  xx %>%
  arrange(Var1) %>%
  set_names(c("year", "EWNI_combined", "EWNI_obs", "exit"))
...
plot(1:nrow(tab) - 1, tab$haz * 100000, type = 'o')
plot(1:nrow(tab) - 1, tab$haz2 * 100000, type = 'o')
