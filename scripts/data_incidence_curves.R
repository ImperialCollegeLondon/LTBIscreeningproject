...
# plot
par(mfcol = c(1, 2))
table(ceiling(IMPUTED_sample$notif_issdt.years)) %>% plot(ylim = c(0,700))#, xlim = c(0,20))
table(ceiling(IMPUTED_sample$notif_issdt.years[IMPUTED_sample$uk_tb_orig == "1"])) %>% lines(col = "red")#, xlim = c(0,20))
table(ceiling(IMPUTED_sample$exituk_tb.years)) %>% plot(ylim = c(0,700))#, xlim = c(0,20))

# table
xx <- merge(merge(as.data.frame(table(ceiling(IMPUTED_sample$notif_issdt.years))),
            as.data.frame(table(ceiling(IMPUTED_sample$notif_issdt.years[IMPUTED_sample$uk_tb_orig == "1"]))), by = "Var1", all = TRUE),
...
# plots
par(mfcol = c(1, 2))
table(ceiling(IMPUTED_sample_year_cohort$notif_issdt.years)) %>% plot(ylim = c(0,100))#, xlim = c(0,20))
table(ceiling(IMPUTED_sample_year_cohort$notif_issdt.years[IMPUTED_sample_year_cohort$uk_tb_orig == "1"])) %>% lines(col = "red", xlim = c(0,20))
table(ceiling(IMPUTED_sample_year_cohort$exituk_tb.years)) %>% plot(ylim = c(0,200))#, xlim = c(0,20))

# table
xx <- merge(merge(as.data.frame(table(ceiling(IMPUTED_sample_year_cohort$notif_issdt.years))),
                  as.data.frame(table(ceiling(IMPUTED_sample_year_cohort$notif_issdt.years[IMPUTED_sample_year_cohort$uk_tb_orig == "1"]))), by = "Var1", all = TRUE),
