  ...
  for (min_screen_length_of_stay in c(0,5)) {
    for (incidence in 1:length(incidence_list)) {
      ...
      programme_level_scenarios <- rbind(programme_level_scenarios,
                                         cbind(formatC(programme_level, width = 3, format = "d", flag = "0"), s, min_screen_length_of_stay, incidence))
...
rm(environ_name, incidence, programme_level, min_screen_length_of_stay, s, incidence_list, programme_level_scenarios)
